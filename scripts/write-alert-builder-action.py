...
p = Path(__file__).resolve().parents[1] / "components/alerts/AlertBuilderActionPanel.tsx"
p.write_text(
    r"""'use client';
...
function GlassCard({
  ...
  return <motionlessGlassCard className={cn('rounded-xl border p-2.5', ring, className)}>{children}</motionlessGlassCard>;
}

function GlassCardDiv({
  children,
  className,
}: {
  children: ReactNode;
  ...
  return <div className={className}>{children}</motionlessGlassCard>;
}
