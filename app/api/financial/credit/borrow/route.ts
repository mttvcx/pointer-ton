...
const Body = z.object({
  ...
  /**
   * Optional: deposit this much collateral (integer, token base units — SOL =
   * lamports = SOL*1e9) in the SAME tx before borrowing (atomic, one signature).
   * Omitted = borrow-only against already-deposited collateral. Backwards-compatible.
   */
  depositBaseUnits: z.string().regex(/^\d+$/).optional(),
});
...
export async function POST(req: NextRequest) {
  ...
  try {
    const built = await buildBorrowTx({
      ...
      depositBaseUnits: parsed.depositBaseUnits,
    });
