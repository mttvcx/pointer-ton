...
import { useAutoBuyToastStore, type AutoBuyToastItem } from '@/store/autoBuyToasts';
      ...
      <div className="min-w-0 flex-1">
        ...
        {isSuccess && item.txSignature ? (
          <a
            ...
            className="mt-1 inline-block text-[11px] text-emerald-400/90 underline-offset-2 hover:underline"
...
function ToastCard({ item }: { item: AutoBuyToastItem }) {
  const dismiss = useAutoBuyToastStore((s) => s.dismiss);
  ...
  const panel = (
    <div className="min-w-[280px] rounded-xl border border-white/[0.08] bg-[#0e0e10] p-3 shadow-2xl shadow-black/60">
      <ToastBody
        item={item}
        isPending={isPending}
        isSuccess={isSuccess}
...
export function AutoBuyToastHost() {
  const items = useAutoBuyToastStore((s) => s.items);
  if (items.length === 0) return null;

  return (
    <div
      className="pointer-events-auto fixed bottom-4 right-4 z-[320] flex flex-col gap-2"
      aria-live="polite"
    >
      {items.map((item) => (
