...
export async function buildBorrowTx(input: {
  ...
  borrowedUsd: number;
  /** Optional: deposit this much collateral (base units) atomically before borrowing. */
  ...
  try {
    ...
    const klend = require('@kamino-finance/klend-sdk') as {
      ...
      KaminoAction: {
        ...
        // Atomic deposit-collateral + borrow in one obligation update (one signature).
    ...
    const obligation = new klend.VanillaObligation(new PublicKey(KLEND_PROGRAM_ID));
    const borrowBaseUnits = String(Math.round(input.amountUsd * 1e6)); // USDC (6dp)

    // With depositBaseUnits → deposit collateral + borrow atomically (one signature),
    // so a brand-new user with an empty obligation still succeeds. Without it →
    // borrow-only against already-deposited collateral (backwards-compatible).
    const action = input.depositBaseUnits
      ? await klend.KaminoAction.buildDepositAndBorrowTxns(
          kmarket,
          input.depositBaseUnits,
      ...
      : await klend.KaminoAction.buildBorrowTxns(
          kmarket,
          borrowBaseUnits,
          new PublicKey(USDC_MINT),
          owner,
